...
/**
 * Use case: Retrieve a paginated list of all notifications.
 *
 * Responsibilities:
 * 1. Retrieve notification data and pagination information from the repository using the provided query.
 * 2. Return a standardized success response containing the notification list and pagination details.
 */
export class RetrieveAllUseCase extends BaseUseCase<IInput, IDeps, ISuccessData> {
  async handle(input: IInput): Promise<IUseCaseOutputSuccess<ISuccessData> | IUseCaseOutputFailed> {
    // 1. Retrieve notification data and pagination information from the repository.
