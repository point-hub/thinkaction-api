...
import { RetrieveAllUseCase } from '../use-cases/retrieve-all.use-case';

export const retrieveAllController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const retrieveAllUseCase = new RetrieveAllUseCase({ retrieveAllRepository });

    // Execute business logic
    const response = await retrieveAllUseCase.handle({ user: controllerInput.req['user'], query: controllerInput.req['query'] });
